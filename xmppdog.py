...
#        return m
        if body:
            cmds=body.split()
            if cmds[0] == "/admin" and len(cmds) >=2 and t=="chat":
                if cmds[1] == "status":
                    p = Presence(status=cmds[2])
                    return [p]

            elif cmds[0] == "/room" and len(cmds) >=2 and t=="chat":
                if cmds[1] == "nick":
                    for rm_state in self.client.room_manager.rooms.values():
...
class VersionHandler(object):
    ...
    def get_version(self,iq):
        ...
        q.newTextChild(q.ns(),"name", u"宠物狗")
...
conf.read("xmppdog.ini")

c=Client(JID(conf.xmppdog.account), conf.xmppdog.password)
